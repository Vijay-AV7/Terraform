...
resource "aws_instance" "instance" { # here aws_instance is resource type (predefined) cannot change. "instance" is resource name and its used to refer the resoruce 
  ...
  ami           = local.ami_id # This is devops-practice AMI ID
  instance_type = local.instance_type #If yes t3.micro else t2.micro
  vpc_security_group_ids = local.security_grp_id
  # tags = {
